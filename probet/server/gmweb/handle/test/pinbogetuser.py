...
def handleHttp(dict_param: dict):
    ...
    登陆应该传递的参数有用户名相关的信息
    """
    testUrl = "https://paapistg.oreo88.com/b2b/player/info?userCode=PSZ4000002"
    ...
    objRsp.data.userCode = dictDataInfo["userCode"]
    objRsp.data.loginId = dictDataInfo["loginId"]
    objRsp.data.firstName = dictDataInfo["firstName"]
    objRsp.data.lastName = dictDataInfo["lastName"]
    objRsp.data.status = dictDataInfo["status"]
    objRsp.data.availableBalance = dictDataInfo["availableBalance"]
    objRsp.data.outstanding = dictDataInfo["outstanding"]
    objRsp.data.createdDate = dictDataInfo["createdDate"]
    objRsp.data.createdBy = dictDataInfo["createdBy"]
